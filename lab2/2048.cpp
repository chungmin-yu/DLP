...
/** TODO
 * This lab is taining for state, not afterstate.
 * g++ -std=c++11 -O3 -o 2048 2048.cpp
 * 1. estimate : estimate the value of a given board
 * 2. update_episode : update the tuple network by an episode
 *    -> call update : update the value of a given board, and return its updated value
 * 3. indexof : return 24-bit index of tuple
 * 4. select_best_move : choose the policy to determine the action
 *    -> call evaluate : calculate the value
 *    -> call evaluate_expectimax : calculate the value using expectimax
 * 5. make_statistic : output the episode & mean & max score
 * 6. main : restore the model from file / store the model into file
**/
...
public:
	...
	void make_statistic(size_t n, const board& b, int score, int unit = 1000) {
		...
		if (n % unit == 0) { // show the training process
			...
			info << std::endl;
			// TODO
			// std::fstream output("record.txt", std::ios::app);
		    // output << n << " " << mean << " " << max << std::endl;
			// output.close();
